...
class Jaffe(LensProfileBase):
    ...
    def mass_3d(self, r, rho0, Rs):
        ...
        """
        mass_3d = 4*np.pi*Rs**3*rho0 * r/(r + Rs)
        return mass_3d
    ...
    def function(self, x, y, sigma0, Rs, center_x=0, center_y=0):
        ...
        X = r / Rs
        f_ = 2 * sigma0 * Rs ** 2 * (np.pi * X + np.log(X*X) - 2 * (X*X -
                                                                    1)*self._F(X))
        return f_
